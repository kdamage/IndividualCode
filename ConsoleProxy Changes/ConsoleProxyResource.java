...
public class ConsoleProxyResource extends ServerResourceBase implements
	...
	private Answer executeProxyLoadScan(final Command cmd,
		...
		/* 	Start hijack <-- this code block tells the cloud to look for a python file in order to populate the 
		variable: externalIp. Purpose is to allow the console proxy to be reached when housed behind a NAT 
		firewall. This hi-jack simply lets the external python module run logic to determine the address to 
	...
	*/
			...
			if (f.exists()) {
				...
				try {
					Process p = Runtime.getRuntime().exec("python /usr/local/cs_python_modules/natchecker.py");
					BufferedReader input = 	new BufferedReader(
											new InputStreamReader(p.getInputStream()));
	    			externalIp = input.readLine();
				} catch (Exception e) {
					e.printStackTrace();
				}
